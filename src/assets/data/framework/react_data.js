export const react_data=[
	{
		...
		content:'生命周期函数，在rander之前执行，用此方法可以先更新data值，然后再在rander中渲染；\n\
		切记：ajax时异步函数，在componentWillMount 中执行ajax，this.setState时，组件data并不会直接改变，而是会在ajax获取会数据后再进行改变data，所以如果render前没有定义data的值时，rander时会报错，data undefined ，所以在使用ajax前需要给data定义。'
	},
	{
